        ...
        if(CircuitAddons.getSettingsFile().getBoolean("guis.alltitle.enabled")) {
            ...
            if (type.equals("title")) {
                ...
                for (Player i : Bukkit.getOnlinePlayers()) {
                    i.sendTitle(title, subtitle, 10, 40, 10);
                    if (!actionbar.isEmpty()) {
                        i.sendActionBar(actionbar); // Now sending action bar to all players
                    }
                }
            } else if (type.equals("subtitle")) {
                for (Player i : Bukkit.getOnlinePlayers()) {
                    i.sendTitle("", fullText, 10, 40, 10); // Fixed: Sending to all players
                }
